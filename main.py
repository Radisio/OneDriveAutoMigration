...
def is_it_exception(path, exception):
    for e in exception:
        if e in path:
            return True
    return False
def switch_drive(source, destination,copy_existing, exception):
...
def read_config():
    ...
    return source, destination,copy_existing, exception

# Press the green button in the gutter to run the script.
if __name__ == '__main__':
    source, dest,copy_existing, exception = read_config()
    switch_drive(source, dest,copy_existing, exception)
